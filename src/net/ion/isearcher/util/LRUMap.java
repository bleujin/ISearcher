 ...
 * 
 * <p>
 * A synchronized version can be obtained with: <code>Collections.synchronizedMap( theMapToSynchronize )</code> If it will be accessed by multiple threads, you _must_ synchronize access to this Map. Even concurrent get(Object) operations produce
 * indeterminate behaviour.
 * </p>
 * 
 * <p>
 * Unlike the Collections 1.0 version, this version of LRUMap does use a true LRU algorithm. The keys for all gets and puts are moved to the front of the list. LRUMap is now a subclass of SequencedHashMap, and the "LRU" key is now equivalent to
 * LRUMap.getFirst().
 * </p>
 * 
...
public class LRUMap extends SequencedHashMap implements Externalizable {
	...
	/**
	 * Create a new LRUMap with a maximum capacity of <i>i</i>. Once <i>i</i> capacity is achieved, subsequent gets and puts will push keys out of the map. See .
	 * 
	 * @param i
	...
	/**
	 * <p>
	 * Get the value for a key from the Map. The key will be promoted to the Most Recently Used position. Note that get(Object) operations will modify the underlying Collection. Calling get(Object) inside of an iteration over keys, values, etc. is
	 * currently unsupported.
	 * </p>
	 * 
	 * @param key
	 *            Key to retrieve
	 * @return Returns the value. Returns null if the key has a null value <i>or</i> if the key has no value.
	 */
