...
/**
 * A map of objects whose mapping entries are sequenced based on the order in which they were added. This data structure has fast <i>O(1)</i> search time, deletion time, and insertion time.
 * 
 * <p>
 * Although this map is sequenced, it cannot implement {@link java.util.List} because of incompatible interface definitions. The remove methods in List and Map have different return values (see: {@link java.util.List#remove(Object)} and
...
public class SequencedHashMap implements Map, Cloneable, Externalizable {
	...
	private static class Entry implements Map.Entry {
		...
		public boolean equals(Object obj) {
			...
			return ((getKey() == null ? other.getKey() == null : getKey().equals(other.getKey())) && (getValue() == null ? other.getValue() == null
	...
	/**
	 * Construct an empty sentinel used to hold the head (sentinel.next) and the tail (sentinel.prev) of the list. The sentinal has a <code>null</code> key and value.
	 **/
	...
	/**
	 * Return the entry for the "oldest" mapping. That is, return the Map.Entry for the key-value pair that was first put into the map when compared to all the other pairings in the map. This behavior is equivalent to using
	...
	/**
	 * Return the key for the "oldest" mapping. That is, return the key for the mapping that was first put into the map when compared to all the other objects in the map. This behavior is equivalent to using <code>getFirst().getKey()</code>, but this
	 * method provides a slightly optimized implementation.
	 * 
	...
	/**
	 * Return the value for the "oldest" mapping. That is, return the value for the mapping that was first put into the map when compared to all the other objects in the map. This behavior is equivalent to using <code>getFirst().getValue()</code>,
	 * but this method provides a slightly optimized implementation.
	 * 
	...
	/**
	 * Return the key for the "newest" mapping. That is, return the key for the mapping that was last put into the map when compared to all the other objects in the map. This behavior is equivalent to using <code>getLast().getKey()</code>, but this
	 * method provides a slightly optimized implementation.
	 * 
	...
	/**
	 * Return the value for the "newest" mapping. That is, return the value for the mapping that was last put into the map when compared to all the other objects in the map. This behavior is equivalent to using <code>getLast().getValue()</code>, but
	 * this method provides a slightly optimized implementation.
	 * 
	...
	/**
	 * Adds all the mappings in the specified map to this map, replacing any mappings that already exist (as per {@link Map#putAll(Map)}). The order in which the entries are added is determined by the iterator returned from {@link Map#entrySet()} for
	 * the specified map.
	 * 
	 * @param t
	...
	/**
	 * Provides a string representation of the entries within the map. The format of the returned string may change with different releases, so this method is suitable for debugging purposes only. If a specific format is required, use
	 * {@link #entrySet()}.{@link Set#iterator() iterator()} and iterate over the entries in the map formatting them as appropriate.
	 **/
	public String toString() {
	...
	private class OrderedIterator implements Iterator {
		/**
		 * Holds the type that should be returned from the iterator. The value should be either {@link #KEY}, {@link #VALUE}, or {@link #ENTRY}. To save a tiny bit of memory, this field is also used as a marker for when remove has been called on the
		 * current object to prevent a second remove on the same element. Essientially, if this value is negative (i.e. the bit specified by {@link #REMOVED_MASK} is set), the current position has been removed. If positive, remove can still be
		 * called.
		 **/
		...
		/**
		 * Construct an iterator over the sequenced elements in the order in which they were added. The {@link #next()} method returns the type specified by <code>returnType</code> which must be either {@link #KEY}, {@link #VALUE}, or {@link #ENTRY}.
		 **/
		 ...
		 * 
		 * @return <code>true</code> if there are more elements left to be returned from the iterator; <code>false</code> otherwise.
		 **/
		...
		public Object next() {
			if (modCount != expectedModCount) {
				throw new ConcurrentModificationException("seqHashMapConcurrentModificationException00");
			}
			if (pos.next == sentinel) {
				throw new NoSuchElementException("seqHashMapNoSuchElementException00");
			}
			...
			default:
				// should never happen
				throw new Error("seqHashMapBadIteratorType01");
			}
		 ...
		 *             if there isn't a "last element" to be removed. That is, if {@link #next()} has never been called, or if {@link #remove()} was already called on the element.
		 * 
		 * @throws ConcurrentModificationException
		 *             if a modification occurs in the underlying map.
		 **/
		public void remove() {
			if ((returnType & REMOVED_MASK) != 0) {
				throw new IllegalStateException("seqHashMapIllegalStateException00");
			}
			if (modCount != expectedModCount) {
				throw new ConcurrentModificationException("seqHashMapConcurrentModificationException00");
			}
	...
	/**
	 * Creates a shallow copy of this object, preserving the internal structure by copying only references. The keys and values themselves are not <code>clone()</code>'d. The cloned object maintains the same sequence.
	 * 
	 ...
	 *             if the specified index is <code>&lt; 0</code> or <code>&gt;</code> the size of the map.
	 **/
	private Map.Entry getEntry(int index) {
		Entry pos = sentinel;

		if (index < 0) {
			throw new ArrayIndexOutOfBoundsException("seqHashMapArrayIndexOutOfBoundsException01");
		}
		...
		if (pos.next == sentinel) {
			throw new ArrayIndexOutOfBoundsException("seqHashMapArrayIndexOutOfBoundsException02");
		}
	...
	/**
	 * Returns a List view of the keys rather than a set view. The returned list is unmodifiable. This is required because changes to the values of the list (using {@link java.util.ListIterator#set(Object)}) will effectively remove the value from the
	 * list and reinsert that value at the end of the list, which is an unexpected side effect of changing the value of a list. This occurs because changing the key, changes when the mapping is added to the map and thus where it appears in the list.
	 * 
	 * <p>
