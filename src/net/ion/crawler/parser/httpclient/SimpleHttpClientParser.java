...
import net.ion.crawler.handler.BinaryHandler;
...
import net.ion.crawler.parser.PageData.DataType;
import net.ion.crawler.parser.link.LinkExtractorBleujin;
import net.ion.crawler.util.HttpClientUtil;
...
public class SimpleHttpClientParser extends HttpClientWrapper implements IParser {
	...
	private HTTPRequest makeGetMethod(Link link) {
		...
		request.addHeader(HeaderConstant.HEADER_USER_AGENT, USER_AGENT);
		if (link.getTimestamp() > 0) {
			request.addHeader(HeaderConstant.HEADER_IF_MODIFIED_SINCE, DateUtils.formatDate(GregorianCalendar.getInstance().getTime()));
		}
	...
	private boolean containsText(HTTPResponse response) {
		Header contentType = response.getFirstHeader(HeaderConstant.HEADER_CONTENT_TYPE);
		if (contentType != null) {
