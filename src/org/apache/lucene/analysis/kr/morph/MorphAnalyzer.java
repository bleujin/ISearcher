...
public class MorphAnalyzer {
		...
		for (AnalysisOutput o : candidates) {
			...
			if (o.getScore() == AnalysisOutput.SCORE_CORRECT && o.getPos() != PatternConstants.POS_NOUN) {
				...
				if (o.getPos() == PatternConstants.POS_NOUN) {
					...
					addResults(o, results, stems);
					noun = o;
				} else if (o.getPatn() == PatternConstants.PTN_N || (o.getPatn() > noun.getPatn()) || (o.getPatn() == noun.getPatn() && o.getJosa() != null && noun.getJosa() != null && o.getJosa().length() > noun.getJosa().length())) {
					results.remove(noun);
					addResults(o, results, stems);
					noun = o;
				}
	...
	/**
	 * 체언 + 조사 (PTN_NJ) 체언 + 용언화접미사 + '음/기' + 조사 (PTN_NSMJ 용언 + '음/기' + 조사 (PTN_VMJ) 용언 + '아/어' + 보조용언 + '음/기' + 조사(PTN_VMXMJ)
	 * 
	 * @param stem
	 * @param end
	 * @param candidates
	...
	public void analysisWithEomi(String stem, String end, List candidates) throws MorphException {
		...
		try {
			...
			if (entry != null && !(("을".equals(end) || "은".equals(end) || "음".equals(end)) && (entry.getFeature(WordEntry.IDX_REGURA) == IrregularUtil.IRR_TYPE_LIUL || entry.getFeature(WordEntry.IDX_REGURA) == IrregularUtil.IRR_TYPE_BIUP))) {
	...
	/**
	 * 복합명사인지 조사하고, 복합명사이면 단위명사들을 찾는다. 복합명사인지 여부는 단위명사가 모두 사전에 있는지 여부로 판단한다. 단위명사는 2글자 이상 단어에서만 찾는다.
	 * 
	 * @param o
	 * @return
	 * @throws MorphException
